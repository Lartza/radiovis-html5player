...
function radiovisplayer_initsocket(topic, host, port) {
	...
	let elem = $('.radiovis-main[radiovis-topic="' + topic + '"]');
	elem.find('.radiovis-innertext').html('Connecting to ' + topic + '...');

	const radiovisplayer_connected_message = 'Connected, waiting for the first message !';

	ws.onmessage = function (evt) 
	{ 
		let elem = $('.radiovis-main[radiovis-topic="' + evt.target.radiovis_topic + '"]');
		

		let message = evt.data;
		let splited_message = message.split(':');
...
(function( $ ) {
	$.fn.radiovisplayer = function(topic, host, port) {
		...
		let frame = '<div class="radiovis-mainframe">';
		...
		this.find('.radiovis-outtertext').bind('scrollage', function() {
			...
			let textWidth = $(this).find('.radiovis-innertext').width();

			let finalPos =  textWidth - boxWidth;

			if (finalPos < 0)
				finalPos = 0;

			let animSpeed = finalPos * 20;
